          ...
          <CardContent className="pt-6">
            ...
            >
              ...
              <div className="flex justify-center py-8 relative z-10">
                <div className="w-32 h-32 rounded-full border-4 border-rotc-accent/40 overflow-hidden bg-rotc-bg flex items-center justify-center shadow-lg">
                  {session.photo_url ? (
                    ...
                    <UserIcon className="h-12 w-12 text-rotc-textMuted" />
                  )}
                </div>
              </div>
              ...
              <div className="text-center px-6 pb-6 space-y-1.5 relative z-10">
                ...
                <p className="text-base font-medium text-rotc-accent tracking-widest">{session.id_number}</p>
                <div className="flex items-center justify-center gap-2 text-sm text-rotc-textMuted mt-2">
                  <span className="uppercase tracking-wider font-semibold">{session.role}</span>
                  {session.platoon && (
                    <>
                ...
                {session.designation && (
                  <p className="text-xs text-rotc-info font-medium mt-1 uppercase tracking-wider">{session.designation}</p>
                )}
              </div>
