...
export default function ArchivesPage() {
  ...
  const [vaultPageSize, setVaultPageSize] = useState(50) // Larger page size to see all files & folders
  ...
  const loadVault = async () => {
    ...
    try {
      const result = await getAdminDocuments({
        search: vaultSearch,
        folder: currentPath, // query the database for the active current directory
        page: vaultPage,
  ...
  const handleDownloadAll = async () => {
    // Downloads all actual files in the current directory
  ...
  const handleCreateFolder = async () => {
    const trimmed = newFolderName.trim().replace(/[\/\\]+$/, '') // Strip trailing slashes
    if (!trimmed) return
  ...
  // Filter out any virtual .keep placeholders of folder registrations in list
          ...
          <CardHeader>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full">
              <h3 className="text-sm font-bold text-rotc-text flex items-center gap-2">
                ≡ƒôé {currentPath ? currentPath.split('/').pop() : 'Root Folder'}
              </h3>
                ...
                return (
                  <>
                    ...
                    <td className="p-4 text-sm text-rotc-textMuted">
                      {isFolder ? 'ΓÇö' : `${(d.file_size / 1024).toFixed(1)} KB`}
                    </td>
                    <td className="p-4 text-sm text-rotc-textMuted">
